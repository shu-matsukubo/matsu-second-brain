...
- 前回同期バージョンは、Git管理された `.deploy/knowledge-viewer-tree.txt` に同じ2行形式で保持する。旧形式の1行だけの値は未同期として扱い、安全に新形式へ移行する。
...
1. 公開確認が成功した後だけ、`.deploy` がなければ作成し、現在の `knowledge` と `indexes` のtree SHAを所定の2行形式と末尾改行で `.deploy/knowledge-viewer-tree.txt` へ書き込む。
