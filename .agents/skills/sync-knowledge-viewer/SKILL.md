...
- 前回同期バージョンは、Git管理された `.codex/state/knowledge-viewer-tree.txt` の1行に保持する。
...
4. 前回値は `git show HEAD:.codex/state/knowledge-viewer-tree.txt` でコミット済みの状態ファイルから読む。状態ファイルがまだ存在しない場合は未同期として扱う。
...
1. 公開確認が成功した後だけ、現在のtree SHAと改行を `.codex/state/knowledge-viewer-tree.txt` へ書き込む。
