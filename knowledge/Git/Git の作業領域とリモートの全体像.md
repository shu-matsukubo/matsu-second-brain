...
コミットの単位を整えるには、ファイル単位または差分のまとまり（hunk）単位でステージする。

```bash
# ファイル単位で追加する
git add path/to/file

# 変更箇所を対話的に確認し、必要な hunk だけ追加する
git add -p

# いったん広く追加した後、特定ファイルをコミット対象から外す
git add .
git restore --staged path/to/local-only-file
```

`git add -p` は、作業ツリーとインデックスの差分を順に表示し、選んだ hunk だけをインデックスへ追加する。複数の作業内容が同じファイルに混在した場合でも、コミットに含める変更を分けながらセルフレビューできる。

`git add .` は、実行したディレクトリ以下の追加・変更・削除をまとめてインデックスへ反映する。意図しないファイルを含める可能性があるため、直後に `git status` と `git diff --staged` で確認する。ローカル専用ファイルが恒常的に不要なら、毎回ステージから外す運用ではなく `.gitignore` などで追跡対象を整理する。

`git restore --staged <file>` はインデックスだけを既定で `HEAD` に戻すため、作業ツリーの編集は失われない。対して、`--staged` を付けない `git restore <file>` は作業ツリーをインデックスの内容へ戻し、未保存の変更を失う可能性があるので区別する。
